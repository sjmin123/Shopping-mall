...
public class LoginController {
    ...
    //로그인
    @PostMapping("/login")
    public String login(@ModelAttribute("userRequestDto") LoginRequestDto loginRequestDto, HttpSession session, Model model) {
        if (loginService.login(loginRequestDto, session, model)) {
            return "/main-page";
        }
        return "redirect:/";
    }
